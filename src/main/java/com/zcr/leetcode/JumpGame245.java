...
public class JumpGame245 {
     ...
     * [2,1,3,1,1,1,1]
     * Step记录当前的步数
     * Curmax记录当前这一步能走到的最远的位置（如：初始值是在idx为0位置，当前第一步所能走到的最远的位置是idx为2；走两步第二步所能走到的最远的位置是idx为5）
     ...
     * Curmax  0     2     5      5
     * Nextmax0->2   2->5  5
     * 当内层循环的idx到6的时候，已经不满足idx<=curMax了，这时候内循环已经结束了，外循环也结束了，返回0。意思是说：永远不可能到达队尾。
     *
